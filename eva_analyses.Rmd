...
cat <- combined %>%
  ...
  select(2:3, 10:13, 15) %>%
  group_by(qualtrics_id, instrument, inst_id, tuning_step) %>%
  count(selected_major)

# since count function dropped rows where n == 0, but we still need those rows to calculate proportion of major cat
# we select rows where n == 8 (all judged as major/ all judged as minor), 
...
rtg <- df_j %>%
  ...
  group_by(qualtrics_id, instrument, inst_id) %>%
  count(explicit_rtg) %>%
  select(-n) 

# df for both combined
cat_rtg <- cat %>% 
  left_join(rtg, by = c("qualtrics_id", "instrument", "inst_id"))
```
