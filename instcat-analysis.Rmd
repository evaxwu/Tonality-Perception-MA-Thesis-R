...
```{r outliers_slope}
slopes_data <- as.data.frame(data.cat$qualtrics_id) %>%
  mutate(slopes = NA) %>%
  rename(qualtrics_id = `data.cat$qualtrics_id`) %>%
  filter(qualtrics_id != "participant") %>%
  unique()

for (i in slopes_data$qualtrics_id) {
  data_filtered <- data.cat %>%
    filter(qualtrics_id == i)
  simple_model <- glm(selected_major ~ tuning_step, data = data_filtered, family = binomial)
  slopes_data$slopes[slopes_data$qualtrics_id == i] = coef(simple_model)[[2]]
}

outliers_slope <- slopes_data %>%
  identify_outliers(slopes)

# examine outliers; they are a combination of 0's and 1's
data.cat %>%
  filter(qualtrics_id %in% outliers_slope$qualtrics_id)
...
link for [logistic regression](http://www.sthda.com/english/articles/36-classification-methods-essentials/151-logistic-regression-essentials-in-r/)
